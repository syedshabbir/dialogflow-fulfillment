...
class CustomerConnectionHandler extends ChatConnectionHandler {
          ...
          return this.router._sendEventToAgent(customer)
            .then(responses => {
              const response = responses[0];
			  	const str = JSON.stringify(response.queryResult.fulfillmentMessages);
	//const fulfillmentMessages = JSON.parse(str);
  //console.log(`  str: ${fulfillmentMessages[0].message}`);
	//console.log(`  str: ${str}`);

              this._respondToCustomer(str, this.socket);
            });
        }
  ...
  _gotCustomerInput (utterance) {
	...
	console.log('_gotCustomerInput:', utterance);
    this.router.customerStore
      ...
      .then(response => {
		...
		//const response = responses[0];
			  	const str = JSON.stringify(response.queryResult.fulfillmentMessages);
				console.log('str:', JSON.stringify(response.queryResult));
        if (response) {
          return this._respondToCustomer(str, this.socket);
        }
      })
      .catch(error => {
        // Log this unspecified error to the console and
